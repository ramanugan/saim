...
class CrudTiposEquipoModal extends ConsumerStatefulWidget {
  const CrudTiposEquipoModal({super.key});

  @override
  ConsumerState<CrudTiposEquipoModal> createState() => _CrudTiposEquipoModalState();
}

class _CrudTiposEquipoModalState extends ConsumerState<CrudTiposEquipoModal> {
  bool _isEditing = false;
  TipoEquipo? _selectedTipoEquipo;
  ...
  void _initControllers() {
    _codigoCtrl = TextEditingController(text: _selectedTipoEquipo?.codigo ?? '');
    _nombreCtrl = TextEditingController(text: _selectedTipoEquipo?.nombre ?? '');
    _descripcionCtrl = TextEditingController(text: _selectedTipoEquipo?.descripcion ?? '');
    _activo = _selectedTipoEquipo?.activo ?? true;
  }
  ...
  void _openForm([TipoEquipo? tipoEquipo]) {
    setState(() {
      _selectedTipoEquipo = tipoEquipo;
      _initControllers();
      _isEditing = true;
    });
  }
  ...
  Future<void> _saveForm() async {
    ...
    final newTipo = TipoEquipo(
      idTipoEquipo: _selectedTipoEquipo?.idTipoEquipo,
    ...
    try {
      if (_selectedTipoEquipo == null) {
        await ref.read(tiposEquipoProvider.notifier).addTipoEquipo(newTipo);
      } else {
        await ref.read(tiposEquipoProvider.notifier).updateTipoEquipo(newTipo);
      }
      _closeForm();
    } catch (e) {
  ...
  Future<void> _toggleStatus(TipoEquipo tipoEquipo) async {
    final action = tipoEquipo.activo ? 'desactivar' : 'activar';
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text('${tipoEquipo.activo ? 'Desactivar' : 'Activar'} Tipo de Equipo'),
        content: Text('¿Estás seguro de que deseas $action este tipo de equipo?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text('Cancelar')),
          TextButton(
            onPressed: () => Navigator.pop(ctx, true), 
            style: TextButton.styleFrom(foregroundColor: AppColors.red),
            child: Text(tipoEquipo.activo ? 'Desactivar' : 'Activar')
          ),
        ],
      ),
    );

    if (confirm == true && tipoEquipo.idTipoEquipo != null) {
      try {
        await ref.read(tiposEquipoProvider.notifier).deleteTipoEquipo(tipoEquipo.idTipoEquipo!);
      } catch (e) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e')));
      }
    }
          ...
          children: [
            Padding(
              ...
              child: Row(
                ...
                children: [
                  Text(
                    _isEditing ? (_selectedTipoEquipo == null ? 'Nuevo Tipo de Equipo' : 'Editar Tipo de Equipo') : 'Gestión de Tipos de Equipo',
  ...
  Widget _buildTable() {
    final tiposAsync = ref.watch(tiposEquipoProvider);
